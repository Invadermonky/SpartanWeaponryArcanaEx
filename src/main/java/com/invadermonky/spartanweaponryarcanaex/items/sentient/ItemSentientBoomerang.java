...
import com.invadermonky.spartanweaponryarcanaex.items.base.ItemBoomerangSE;
...
public class ItemSentientBoomerang extends ItemBoomerangSE implements ISpartanWillWeapon {
    ...
    public ItemSentientBoomerang() {
        ...
        this.damageMultiplier = ConfigHandler.damageMultiplierBoomerang;
        this.baseAttackDamage = Math.max(0.5F, this.materialEx.getAttackDamage() * this.damageMultiplier + ConfigHandler.damageBaseBoomerang - 1.0F);
        this.baseAttackSpeed = ConfigHandler.meleeSpeedBoomerang;
    }
        ...
        if(stack.getItem() instanceof ISpartanWillWeapon) {
            ...
            if(willBracket >= 0) {
                switch (type) {
                    ...
                    case VENGEFUL:
                        return (int) Math.ceil((double) super.getMaxChargeTicks(stack) * (1 + LibAttributes.Ranged.vengefulChargeTicksModifier[willBracket]));
                }
            }
        }
