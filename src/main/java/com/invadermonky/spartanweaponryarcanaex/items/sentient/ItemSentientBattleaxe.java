...
import com.invadermonky.spartanweaponryarcanaex.items.base.ItemBattleaxeSE;
...
public class ItemSentientBattleaxe extends ItemBattleaxeSE implements ISpartanWillWeapon {
    ...
    public ItemSentientBattleaxe() {
        ...
        this.damageMultiplier = ConfigHandler.damageMultiplierBattleaxe;
        this.baseAttackDamage = Math.max(0.5F, this.materialEx.getAttackDamage() * this.damageMultiplier + ConfigHandler.damageBaseBattleaxe - 1.0F);
    ...
    public void recalculatePowers(ItemStack stack, World world, EntityPlayer player) {
        ISpartanWillWeapon.super.recalculatePowers(stack, world, player);
        EnumDemonWillType type = PlayerDemonWillHandler.getLargestWillType(player);
        double soulsRemaining = PlayerDemonWillHandler.getTotalDemonWill(type, player);
        int level = this.getLevel(stack, soulsRemaining);
        this.setDigSpeedOfWeapon(stack, level >= 0 ? this.getDigSpeed(type, level) : 0.0);
    }
    ...
    public float getDestroySpeed(ItemStack stack, IBlockState state) {
        ...
        return destroySpeed + (float) this.getDigSpeedOfWeapon(stack);
    }

    public double getDigSpeed(EnumDemonWillType type, int willBracket) {
        return LibAttributes.Swords.defaultDigSpeedAdded[willBracket];
    }

    public double getDigSpeedOfWeapon(ItemStack stack) {
        NBTHelper.checkNBT(stack);
        NBTTagCompound tag = stack.getTagCompound();
        return tag.getDouble(Constants.NBT.SOUL_SWORD_DIG_SPEED);
    }

    public void setDigSpeedOfWeapon(ItemStack stack, double speed) {
        NBTHelper.checkNBT(stack);
        NBTTagCompound tag = stack.getTagCompound();
